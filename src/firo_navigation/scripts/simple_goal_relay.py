...
class SimpleGoalRelay:
    ...
    def __init__(self, name="simple_goal_relay"):
        ...
        self.move_base_action = SimpleActionClient(
            "move_base/move_base", MoveBaseAction)
    ...
    def goal_callback(self, msg):
        ...
        self.move_base_action.send_goal(
            MoveBaseGoal(target_pose=msg), self.move_base_done)

    def move_base_done(self, status, result):
        rospy.logdebug(
            "Move base action completed with result [%d]: %s", result.outcome, result.message)
